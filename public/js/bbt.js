// const classes = ['amy', 'sheldon', 'arman', 'leonard']
// const classes = ['119200001', '119200002']

function getFaceImageUri(className, idx) {
  return `${className}/${className}${idx}.png`
}

// function renderFaceImageSelectList(selectListId, onChange, initialValue) {
//   const indices = [1, 2, 3, 4, 5]
//   function renderChildren(select) {
//     classes.forEach(className => {
//       const optgroup = document.createElement('optgroup')
//       optgroup.label = className
//       select.appendChild(optgroup)
//       indices.forEach(imageIdx =>
//         renderOption(
//           optgroup,
//           `${className} ${imageIdx}`,
//           getFaceImageUri(className, imageIdx)
//         )
//       )
//     })
//   }

//   renderSelectList(
//     selectListId,
//     onChange,
//     getFaceImageUri(initialValue.className, initialValue.imageIdx),
//     renderChildren
//   )
// }

// fetch first image of each class and compute their descriptors
async function createBbtFaceMatcher(numImagesForTraining = 1) {
  const maxAvailableImagesPerClass = 5
  numImagesForTraining = Math.min(numImagesForTraining, maxAvailableImagesPerClass)

  var requestOptions = {
    method: 'GET',
    redirect: 'follow'
  };

  var classes = null,
      file = null;

  await fetch("/train-class", requestOptions)
  .then(response => response.json())
  .then(result => {
    classes = result.data.map(String)
    file = result.file
  })
  .catch(error => console.log('error', error));

  // console.log(classes, file)

  var dump = null
  var j = -1;

  const labeledFaceDescriptors = await Promise.all(classes.map(
    async className => {
      const descriptors = []
      for (let i = 1; i < (numImagesForTraining + 1); i++) {
      // for (let i = 1; i < (file.length); i++) {
        // const img = await faceapi.fetchImage(`${className}/${className}${i}.png`)
        // console.error(className, i)
          j = j+1
        if (dump == className) {
          // console.log("sama")
        }else{
          dump = className
          // j = 0
        }
        // console.info(dump, j)
        const img = await faceapi.fetchImage(`${file[j]}`)
        descriptors.push(await faceapi.computeFaceDescriptor(img))
      }
      // console.log(descriptors)
      return new faceapi.LabeledFaceDescriptors(
        className, descriptors
      )
    }
  ))

  return new faceapi.FaceMatcher(labeledFaceDescriptors)
}

module.exports = { getFaceImageUri };